...
class DetailsPanel extends React.Component {
  ...
  componentDidMount() {
    ...
    this.pinAllShownJobsUnlisten = this.$rootScope.$on(thEvents.pinAllShownJobs, () => {
      this.pinAllShownJobs();
    });
  ...
  pinJobs(jobsToPin) {
    const { pinnedJobs } = this.state;
    const newPinnedJobs = jobsToPin.reduce((acc, job) => ({ ...acc, [job.id]: job }), {});
  ...
  pinAllShownJobs() {
    ...
    const showError = shownJobs > spaceRemaining;
  ...
  render() {
    ...
    const {
      ...
      classifications, lvUrl, lvFullUrl, pinnedJobs, pinnedJobBugs, bugs, reftestUrl,
    } = this.state;
